...
InfoLoggerMessageHelper::InfoLoggerMessageHelper()
{
  ...
  if (findIndex(severity) == -1)
    throw __LINE__;
  if (findIndex(level) == -1)
    throw __LINE__;
  if (findIndex(timestamp) == -1)
    throw __LINE__;
  if (findIndex(hostname) == -1)
    throw __LINE__;
  if (findIndex(rolename) == -1)
    throw __LINE__;
  if (findIndex(pid) == -1)
    throw __LINE__;
  if (findIndex(username) == -1)
    throw __LINE__;
  if (findIndex(system) == -1)
    throw __LINE__;
  if (findIndex(facility) == -1)
    throw __LINE__;
  if (findIndex(detector) == -1)
    throw __LINE__;
  if (findIndex(partition) == -1)
    throw __LINE__;
  if (findIndex(run) == -1)
    throw __LINE__;
  if (findIndex(errcode) == -1)
    throw __LINE__;
  if (findIndex(errline) == -1)
    throw __LINE__;
  ...
  switch (format) {
    ...
    case InfoLoggerMessageHelper::Format::Full:
      ...
      for (i = 0; i < protocols[0].numberOfFields; i++) {
        if ((i == ix_timestamp) || (i == ix_severity) || (i == ix_message)) {
          continue;
        }
        if ((i != ix_facility) && (i != ix_pid) && (i != ix_run) && (!all)) {
          continue;
        }
        appendStringSeparator(buffer, bufferSize, &ix);
