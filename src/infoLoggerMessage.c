...
int infoLog_msg_encode(infoLog_msg_t* msg, char* buffer, int bufferSize, int splitLinesForThisFieldIndex)
{
  ...
  if (splitLinesForThisFieldIndex < -1) {
    return __LINE__;
  }
  if (splitLinesForThisFieldIndex >= 0) {
    if (splitLinesForThisFieldIndex >= msg->protocol->numberOfFields) {
      return __LINE__;
    }
    if (msg->protocol->fields[splitLinesForThisFieldIndex].type != ILOG_TYPE_STRING) {
      return __LINE__;
    }
    ...
    */
    ...
    if ((s.isError)                                                                   /* could not complete record */
      ...
      if ((ix >= 0) && ((i + 1) >= msg->protocol->numberOfFields)) {
        ...
        if (ix_truncateMsg < ix) {
          ix_truncateMsg = -1;
        } /* we don't have even space for truncate warning */
      }
      ...
      if ((ix_truncateMsg < 0) || ((int)(ix_truncateMsg + sizeof(truncateMsg)) > bufferSize)) {
        isError = __LINE__;
      } else {
        s.isError = 0;
...
int infoLog_proto_init()
{
  ...
  for (i = 0; i < sizeof(protocols) / sizeof(infoLog_msgProtocol_t); i++) {
    if (protocols[i].version == NULL) {
      return __LINE__;
    }
    if (protocols[i].numberOfFields <= 0) {
      return __LINE__;
    }
    if (protocols[i].numberOfFields > INFOLOG_FIELDS_MAX) {
      return __LINE__;
    }
    for (j = 0; j < protocols[i].numberOfFields; j++) {
      if (protocols[i].fields[j].name == NULL) {
        return __LINE__;
      }
      if ((protocols[i].fields[j].type != ILOG_TYPE_STRING) && (protocols[i].fields[j].type != ILOG_TYPE_INT) && (protocols[i].fields[j].type != ILOG_TYPE_DOUBLE)) {
        return __LINE__;
      }
    }
    ...
    if (strcmp(protocols[i].fields[j].name, "message")) {
      return __LINE__;
    }
    ...
    for (j = 0; j < protocols[i].numberOfFields; j++) {
      ...
      for (k = 0; k < protocols[0].numberOfFields; k++) {
        if (protocols[0].fields[k].type != protocols[i].fields[j].type)
          continue;
        if (strcmp(protocols[0].fields[k].name, protocols[i].fields[j].name))
          continue;
        found = 1;
        break;
      }
...
int infoLog_msg_convert(infoLog_msg_t* msg)
{
  ...
  if (msg->protocol == &protocols[0])
    return 0; /* Nothing to do if no conversion needed */
  if (!infolog_msg_isInit) {
    return __LINE__;
  }
  if (msg->protocol == NULL) {
    return __LINE__;
...
int infoLog_msg_findField(const char* fieldName)
{
  ...
  for (i = 0; i < protocols[0].numberOfFields; i++) {
    if (protocols[0].fields[i].name == NULL) {
      continue;
    }
    if (!strcmp(protocols[0].fields[i].name, fieldName)) {
      return i;
    }
  }
  return -1;
